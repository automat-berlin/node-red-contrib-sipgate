...
describe('dial node', function() {
  ...
  it('should respond with proper XML for target number and onHangup callback', function(done) {
    var flow = [{ id: "n1", type: "dial", target: 'number', numbers: [{ number: '4915799912345' }], onHangup: "http://example.com/callback" }];
    var xml = fs.readFileSync('test/resources/xml/dial_number_onHangup.xml', 'utf8');
    helper.load(dialNode, flow, function() {
      var n1 = helper.getNode("n1");
      n1.on("input", function(msg) {
        should(msg.res._res.responseBody).be.eql(xml);
  ...
  it('should respond with proper XML for target number and onAnswer callback', function(done) {
    var flow = [{ id: "n1", type: "dial", target: 'number', numbers: [{ number: '4915799912345' }], onAnswer: "http://example.com/callback" }];
    var xml = fs.readFileSync('test/resources/xml/dial_number_onAnswer.xml', 'utf8');
    helper.load(dialNode, flow, function() {
      var n1 = helper.getNode("n1");
      n1.on("input", function(msg) {
        should(msg.res._res.responseBody).be.eql(xml);
